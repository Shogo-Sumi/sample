...
class DefaultMap extends Map {
  //DefaultMapクラスを宣言。Mapオブジェクトを継承して、DefaultMapを宣言。Mapオブジェクトはキーと値の組み合わせを保持する。
...
class Histogram {
  ...
  toString() {
    //ヒストグラムを文字列に変換して、ＡＳＣＩＩグラフィックとして表示
    //マップを、[キー、文字列]配列へ変換
    let entries = [...this.letterCounts]; //letterCountsのすべての要素を変数entriesへ格納

    entries.sort((a, b) => {
      //文字数順にソート。文字数が同じ場合は、アルファベット順でソート
      if (a[1] === b[1]) {
        //要素a[1]とb[1]が同値の場合
...
async function histogramFromStdin() {
  //ヒストグラムオブジェクトを生成。標準入力からテキストを非同期に読み出し、読みだしたテキストをヒストグラムに追加。最後まで読みだしたらヒストグラムを返す。asyncは非同期関数
